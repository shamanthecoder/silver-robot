...
public class MinimumSwapSortArray {
	...
	/*
	 * The minimum swaps required is calculated using the cycles in the array. As
	 * the input elements are 1<=i<=n we can use the target indexes to create a
	 * cycle and elements within a cycle will be swapped. Total swaps will be
	 * summation of all the swaps within a cycle. So we find all the cycles covering
	...
	private static int minimumSwapsRequired(int[] input) {
		...
		for (int i = 0; i < input.length; i++) {
			...
			// Add the swaps for a cycle to the answer as answer is summation of all swaps
			// in all cycles.
			if (cycle != 0)
				answer += cycle - 1;
		}
